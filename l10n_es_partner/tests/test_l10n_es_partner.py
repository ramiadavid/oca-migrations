...
class TestL10nEsPartner(common.TransactionCase):
    ...
    def test_search_commercial(self):
        ...
        self.assertIn(self.partner.id, map(lambda x: x[0], partner_obj.name_search("prueba")))
        self.assertNotIn(
            self.partner.id,
            map(lambda x: x[0], partner_obj.name_search("prueba", operator="not ilike")),
        )
        self.assertIn(self.partner.id, map(lambda x: x[0], partner_obj.name_search("comercial")))
        self.assertNotIn(
            self.partner.id,
            map(
                lambda x: x[0],
    ...
    def test_name(self):
        ...
        partner2.with_context(show_address=True, display_commercial=True)._compute_complete_name()
