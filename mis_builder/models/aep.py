...
class AccountingExpressionProcessor:
        ...
        for key, acc_domains in self._map_account_ids.items():
            ...
            for acc_domain in acc_domains:
                ...
                for company in self.companies:
                    acc_domain_with_company = Domain.AND([acc_domain, [("company_ids", "=", company.id)]])
                    account_ids += self._account_model.with_company(company).search(acc_domain_with_company).ids
    ...
    def do_queries(
        ...
        for key in self._map_account_ids:
            ...
            try:
                accs = aml_model.with_context(allowed_company_ids=self.companies.ids)._read_group(
                    domain,
            ...
            for account_id, company_id, debit, credit, *custom_fields_sums in accs:
                ...
                for custom_field, custom_field_sum in zip(self._custom_fields, custom_fields_sums, strict=True):
                    account_data.add_custom_field(custom_field, custom_field_sum or AccountingNone)
