    ...
    def _check_name(self):
        for record in self:
            if not _is_valid_python_var(record.name):
                raise ValidationError(self.env._("KPI name (%s) must be a valid python identifier", record.name))
...
class MisReportKpiExpression(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            if subkpi:
                name = f"{kpi.description} / {subkpi.description} " f"({kpi.name}.{subkpi.name})"
            else:
    ...
    def _search_display_name(self, operator, value):
        if "." in value:
            ...
            name_search_domain = Domain("kpi_id.name", operator, value) | Domain("kpi_id.description", operator, value)
...
class MisReportQuery(models.Model):
    ...
    def _check_name(self):
        for record in self:
            if not _is_valid_python_var(record.name):
                raise ValidationError(self.env._("Query name (%s) must be valid python identifier", record.name))
