...
class KpiMatrix:
    ...
    def compute_sums(self):
        ...
        for (
            ...
            if any(c.subkpis for c in sumcols) and not common_subkpis:
                raise UserError(
                    self.env._(
                        "Sum cannot be computed in column %s " "because the columns to sum have no " "common subkpis",
                        label,
                    )
                )
    ...
    def _get_account_name(self, account):
        ...
        account_companies = account.company_ids & self._companies if self._companies else account.company_ids
    ...
    @classmethod
    def _make_cell_id(cls, kpi_id: int, account_id: int | None, period_id: int, subkpi_id: int | None) -> str:
