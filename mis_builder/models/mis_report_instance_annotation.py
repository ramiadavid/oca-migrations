...
class MisReportInstanceAnnotation(models.Model):
    ...
    def _get_first_matching_annotation(self, cell_id, instance_id):
        ...
        annotation_context = self.env["mis.report.instance"].browse(instance_id)._get_annotation_context()
        annotation_rec = annotations.filtered(lambda rec: rec.annotation_context == annotation_context)
        annotation = next(iter(annotation_rec), annotation_rec)
        return annotation

    @api.model
    def set_annotation(self, cell_id, instance_id, note):
        if not self.env["mis.report.instance"].browse(instance_id).user_can_edit_annotation:
            raise AccessError(self.env._("You do not have the rights to edit annotations"))
        ...
        if annotation:
            ...
            kpi_id, _account_id, period_id, subkpi_id = KpiMatrix._unpack_cell_id(cell_id)
