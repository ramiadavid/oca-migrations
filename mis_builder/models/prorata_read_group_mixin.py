...
class ProRataReadGroupMixin(models.AbstractModel):
    ...
    date = fields.Date(
        ...
        search="_search_date",
        help=("Dummy field that adapts searches on date to searches on date_from/date_to."),
    )
    ...
    def _search_date(self, operator, value):
        ...
        raise UserError(self.env._("Unsupported operator %s for searching on date", operator))
    ...
    def _read_group(
        ...
        if (
            ...
            for item in self.search_fetch(domain, read_fields):
                ...
                res[key] += SimpleArray(self._prorata(item, dt_from, dt_to, f) for f in stripped_aggregates)
