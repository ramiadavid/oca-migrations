...
class TestInvoiceRefundLinkBase(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env["account.move.reversal"].with_context(active_ids=cls.invoice.ids, active_model="account.move").create(
            {
    ...
    def _test_refund_link(self):
        ...
        self.assertEqual(self.invoice.invoice_line_ids[1], refund.invoice_line_ids[1].origin_line_id)
        self.assertTrue(refund.invoice_line_ids[2].origin_line_id)
        self.assertEqual(self.invoice.invoice_line_ids[2], refund.invoice_line_ids[2].origin_line_id)
...
class TestInvoiceRefundLink(TestInvoiceRefundLinkBase):
    ...
    def test_invoice_copy(self):
        refund = self.invoice.refund_invoice_ids[0]
        self.invoice.copy()
        self.assertEqual(refund.invoice_line_ids.origin_line_id, self.invoice.invoice_line_ids)

    def test_refund_copy(self):
        refund = self.invoice.refund_invoice_ids[0]
        refund.copy()
        self.assertEqual(self.invoice.invoice_line_ids, refund.invoice_line_ids.origin_line_id)
