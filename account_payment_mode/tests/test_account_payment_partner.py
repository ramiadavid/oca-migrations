...
class TestAccountPaymentPartner(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        if not chart:
            raise ValidationError(cls.env._("No Chart of Account Template has been defined !"))
    ...
    def test_refund_no_payment_mode_preserves_partner_bank(self):
        ...
        self.company.keep_partner_bank_without_payment_mode = True
        partner_no_mode = (
            self.env["res.partner"].with_company(self.company.id).create({"name": "Partner without payment mode"})
        )
        trusted_bank = self.env["res.partner.bank"].create(
            {
                "account_number": "BE32121212121212",
                "partner_id": partner_no_mode.id,
                "allow_out_payment": True,
            }
        )
        invoice = self._create_invoice(default_move_type="out_invoice", partner=partner_no_mode)
        invoice.payment_mode_id = False
        invoice.action_post()
