...
class AccountPaymentMode(models.Model):
    ...
    payment_type = fields.Selection(related="payment_method_id.payment_type", readonly=True, store=True)
    payment_method_code = fields.Char(related="payment_method_id.code", readonly=True, store=True)
        ...
        for mode in self.filtered(lambda x: x.bank_account_link == "fixed"):
            if not mode.fixed_journal_id:
                ...
                if mode.payment_method_id.payment_type == "outbound":
                    p_modes = f_journal.outbound_payment_method_line_ids.mapped("payment_method_id.id")
                    ...
                    p_modes = f_journal.inbound_payment_method_line_ids.mapped("payment_method_id.id")
                    if mode.payment_method_id.id not in p_modes:
                        raise ValidationError(
                            self.env._(
                                "On the payment mode %(paymode)s, the payment method "
