...
class AccountPaymentLineCreate(models.TransientModel):
    ...
    order_id = fields.Many2one(comodel_name="account.payment.order", string="Payment Order")
    journal_ids = fields.Many2many(comodel_name="account.journal", string="Journals Filter")
    ...
    def default_get(self, field_list):
        ...
        assert context.get("active_model") == "account.payment.order", "active_model should be payment.order"
        ...
        res.update(
            {
                "journal_ids": [(6, 0, mode.default_journal_ids.ids)] if mode.default_journal_ids else False,
    ...
    def _compute_move_line_domain(self):
        ...
        if self.payment_mode:
            if self.payment_mode == "same":
                domain.append(("payment_mode_id", "=", self.order_id.payment_mode_id.id))
