...
class AccountPaymentOrder(models.Model):
    ...
    company_id = fields.Many2one(related="payment_mode_id.company_id", store=True, readonly=True)
    company_currency_id = fields.Many2one(related="payment_mode_id.company_id.currency_id", store=True, readonly=True)
    bank_account_link = fields.Selection(related="payment_mode_id.bank_account_link", readonly=True)
    ...
    total_company_currency = fields.Monetary(compute="_compute_total", store=True, currency_field="company_currency_id")
    ...
    move_count = fields.Integer(compute="_compute_move_count", string="Number of Journal Entries")
    ...
    def _unlink_except_order_uploaded(self):
        for order in self:
            if order.state == "uploaded":
                raise UserError(
                    self.env._("You cannot delete an uploaded payment order. You can " "cancel it in order to do so.")
                )

    @api.constrains("payment_type", "payment_mode_id")
    def payment_order_constraints(self):
        for order in self:
            if order.payment_mode_id.payment_type and order.payment_mode_id.payment_type != order.payment_type:
        ...
        for order in self:
            if order.date_scheduled:
                if order.date_scheduled < today:
                    raise ValidationError(
                        self.env._(
                            "On payment order %(porder)s, the Payment Execution Date " "is in the past (%(exedate)s).",
                            porder=order.name,
    ...
    def _compute_total(self):
        for rec in self:
            rec.total_company_currency = sum(rec.mapped("payment_line_ids.amount_company_currency") or [0.0])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = self.env["ir.sequence"].next_by_code("account.payment.order") or "New"
    ...
    def draft2open(self):
        ...
        for order in self:
            if not order.journal_id:
                raise UserError(self.env._("Missing Bank Journal on payment order %s.", order.name))
            if order.payment_method_id.bank_account_required and not order.journal_id.bank_account_id:
                raise UserError(
                    self.env._(
            ...
            if not order.payment_line_ids:
                raise UserError(self.env._("There are no transactions on payment order %s.", order.name))
            ...
            for payline in order.payment_line_ids:
                ...
                # Group options
                hashcode = payline.payment_line_hashcode() if order.payment_mode_id.group_lines else payline.id
            ...
            if payline_err_text:
                raise UserError(self.env._("There's at least one validation error:\n") + "\n".join(payline_err_text))
    ...
    def generate_payment_file(self):
        ...
        if self.payment_method_id.code == "manual":
            ...
            raise UserError(
                self.env._(
                    "No handler for this payment method. Maybe you haven't " "installed the related Odoo module."
                )
            )
    ...
    def post_and_reconcile(self):
        ...
        for payment in self.payment_ids:
            (
                ...
                + payment.move_id.line_ids.filtered(lambda x, p=payment: x.account_id == p.destination_account_id)
            ).reconcile()

    def generated2uploaded(self):
        self.post_and_reconcile()
        self.write({"state": "uploaded", "date_uploaded": fields.Date.context_today(self)})
        return True
