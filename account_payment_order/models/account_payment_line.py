...
class AccountPaymentLine(models.Model):
    ...
    company_currency_id = fields.Many2one(related="order_id.company_currency_id", store=True, readonly=True)
    payment_type = fields.Selection(related="order_id.payment_type", store=True, readonly=True)
    bank_account_required = fields.Boolean(related="order_id.payment_method_id.bank_account_required", readonly=True)
    state = fields.Selection(related="order_id.state", string="State", readonly=True, store=True)
    ...
    partner_bank_acc_type = fields.Selection(related="partner_bank_id.account_type", string="Bank Account Type")
    ...
    communication = fields.Char(required=False, help="Label of the payment that will be seen by the destinee")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = self.env["ir.sequence"].next_by_code("account.payment.line") or "New"
    ...
    def draft2open_payment_line_check(self):
        ...
        if self.bank_account_required and not self.partner_bank_id:
            raise UserError(self.env._("Missing Partner Bank Account on payment line %s", self.name))
        if not self.communication:
            raise UserError(self.env._("Communication is empty on payment line %s.", self.name))
    ...
    def _prepare_account_payment_vals(self):
        ...
        if not vals["destination_account_id"]:
            if vals["partner_type"] == "customer":
                vals["destination_account_id"] = self.partner_id.property_account_receivable_id.id
            else:
