...
class AccountMoveLine(models.Model):
    ...
    def _compute_partner_bank_id(self):
        for ml in self:
            if (
                ...
                and not any(
                    p_state in ("draft", "open", "generated") for p_state in ml.payment_line_ids.mapped("state")
                )
            ):
    ...
    def _prepare_payment_line_vals(self, payment_order):
        ...
        partner_bank_id = self.partner_bank_id.id or (first_bank.id if first_bank else False)
        vals = {
