...
class AccountMove(models.Model):
    ...
    def _compute_payment_order_ok(self):
        for move in self:
            payment_mode = move.line_ids.filtered(lambda x: not x.reconciled).mapped("payment_mode_id")[:1]
            if not payment_mode:
    ...
    def _compute_payment_line_count(self):
        for move in self:
            move.payment_line_count = len(
                self.env["account.payment.line"]._search([("move_line_id", "in", self.line_ids.ids)])
            )
    ...
    def _get_payment_order_communication_full(self):
        ...
        if self.reversal_move_ids:
            references.extend([move._get_payment_order_communication_direct() for move in self.reversal_move_ids])
    ...
    def create_account_payment_line(self):
        ...
        for move in self:
            if move.state != "posted":
                raise UserError(self.env._("The invoice %s is not in Posted state", move.name))
            pre_applicable_lines = move.line_ids.filtered(
                lambda x: (not x.reconciled and x.account_id.account_type in ("asset_receivable", "liability_payable"))
            )
            if not pre_applicable_lines:
                raise UserError(self.env._("No pending AR/AP lines to add on %s", move.name))
            ...
            applicable_lines = pre_applicable_lines.filtered(lambda x: x.payment_mode_id.payment_order_ok)
            ...
            for payment_mode in payment_modes:
                payorder = apoo.search(move.get_account_payment_domain(payment_mode), limit=1)
                new_payorder = False
                if not payorder:
                ...
                count = 0
                for line in applicable_lines.filtered(lambda x, mode=payment_mode: x.payment_mode_id == mode):
        ...
        action_xml_id = f"account_payment_order.account_payment_order_{action_payment_type}_action"
    ...
    def action_payment_lines(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id("account_payment_order.account_payment_line_action")
        action.update(
            {
