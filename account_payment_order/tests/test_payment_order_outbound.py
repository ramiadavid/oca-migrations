...
class TestPaymentOrderOutboundBase(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env.user.group_ids |= cls.env.ref("account_payment_order.group_account_payment")
...
class TestPaymentOrderOutbound(TestPaymentOrderOutboundBase):
    ...
    def order_creation(self, date_prefered):
        ...
        line_create = (
            ...
            .create({"date_type": "move", "filter_date": datetime.now() + timedelta(days=1)})
        )
        ...
        line_created_due = (
            self.env["account.payment.line.create"]
            .with_context(active_model="account.payment.order", active_id=order.id)
            .create({"date_type": "due", "filter_date": datetime.now() + timedelta(days=1)})
        )
    ...
    def test_invoice_communication_01(self):
        self.assertEqual("F1242", self.invoice._get_payment_order_communication_direct())
        self.invoice.ref = "F1243"
        self.assertEqual("F1243", self.invoice._get_payment_order_communication_direct())

    def test_invoice_communication_02(self):
        self.assertEqual("F1242", self.invoice._get_payment_order_communication_direct())
        self.invoice.payment_reference = "R1234"
        self.assertEqual("R1234", self.invoice._get_payment_order_communication_direct())
    ...
    def test_invoice_communication_03(self):
        ...
        reverse_wizard = Form(
            self.env["account.move.reversal"].with_context(active_ids=self.invoice.ids, active_model=self.invoice._name)
        )
    ...
    def test_supplier_invoice_payment_reference(self):
        ...
        self.invoice.action_post()
        self.assertEqual("+++F1234+++", self.invoice._get_payment_order_communication_full())
    ...
    def test_manual_line_and_manual_date(self):
        ...
        self.assertEqual(outbound_order.payment_line_ids[0].date, date.today() + timedelta(days=8))
        ...
        self.assertEqual(outbound_order.payment_line_ids[0].payment_ids.date, fields.Date.today())
    ...
    def test_supplier_refund_reference(self):
        ...
        self.invoice.action_post()
        self.assertEqual("F/1234", self.invoice._get_payment_order_communication_direct())
        self.refund = self._create_supplier_refund(self.invoice)
        with Form(self.refund) as refund_form:
            refund_form.ref = "R1234"
        ...
        self.refund.action_post()
        self.assertEqual("FR/1234", self.refund._get_payment_order_communication_direct())
