...
class TestAccountPayment(AccountTestInvoicingCommon):
    ...
    def test_account_payment_03(self):
        ...
        new_account_payment = self.account_payment_model.with_context(default_company_id=self.company.id).new(
            {
        ...
        payment_methods = (
            new_account_payment.available_payment_method_line_ids.filtered(lambda x: x.payment_type == "inbound")
        ...
        # check payment methods
        new_account_payment2 = self.account_payment_model.with_context(default_company_id=self.company.id).new(
            {
                "journal_id": self.bank_journal.id,
                "payment_type": "inbound",
                "amount": 1,
        ...
        payment_methods = new_account_payment2.available_payment_method_line_ids.mapped("payment_method_id").ids
        ...
        # check payment methods
        new_account_payment3 = self.account_payment_model.with_context(default_company_id=self.company.id).new(
            {
                "journal_id": self.bank_journal.id,
                "payment_type": "inbound",
                "amount": 1,
        ...
        payment_methods = new_account_payment3.available_payment_method_line_ids.mapped("payment_method_id").ids
        self.assertNotIn(self.inbound_payment_method_01.id, payment_methods)
