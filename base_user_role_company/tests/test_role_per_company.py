...
class TestUserRoleCompany(TransactionCase):
    ...
    def test_110_company_1(self):
        ...
        self.test_user.with_context(active_company_ids=self.company1.ids).set_groups_from_roles()
    ...
    def test_120_company_2(self):
        ...
        self.test_user.with_context(active_company_ids=self.company2.ids).set_groups_from_roles()
    ...
    def test_130_all_company(self):
        ...
        self.test_user.with_context(active_company_ids=[self.company1.id, self.company2.id]).set_groups_from_roles()
        enabled = self.test_user.group_ids
        expected = self.groupA | self.groupC
        found = enabled.filtered(lambda x: x in expected)
        self.assertEqual(expected, found)
