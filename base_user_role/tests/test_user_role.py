...
class TestUserRole(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_id = cls.user_model.create({"name": "USER TEST (ROLES)", "login": "user_test_roles"})
        ...
        vals = {
            "name": "ROLE_1",
            "implied_ids": [fields.Command.set([cls.group_user_id.id, cls.group_no_one_id.id])],
        }
        ...
        cls.multicompany_user_1 = cls.user_model.create(
            {
                ...
                "group_ids": [fields.Command.set(cls.env.ref("base.group_erp_manager").ids)],
        ...
        cls.multicompany_role = cls.role_model.create(
            {
                ...
                "line_ids": [fields.Command.create({"user_id": cls.multicompany_user_2.id})],
            }
        )

    def test_role_1(self):
        self.user_id.write({"role_line_ids": [fields.Command.create({"role_id": self.role1_id.id})]})
    ...
    def test_role_2(self):
        self.user_id.write({"role_line_ids": [fields.Command.create({"role_id": self.role2_id.id})]})
        user_group_ids = sorted({group.id for group in self.user_id.group_ids})
        ...
        self.user_id.write(
            {
                "role_line_ids": [
                    ...
                    fields.Command.create({"role_id": self.role1_id.id, "date_from": today_str}),
                    # Role 2 should be disabled
                    fields.Command.create({"role_id": self.role2_id.id, "date_to": yesterday_str}),
                ]
            }
        )
        user_group_ids = sorted({group.id for group in self.user_id.group_ids})
    ...
    def test_role_line_unlink(self):
        ...
        self.user_id.role_line_ids.filtered(lambda rl: rl.role_id.id == self.role2_id.id).unlink()
        # Check user has groups from only role1
        self.assertLessEqual(role1_groups, self.user_id.group_ids)
        self.assertFalse(role2_groups <= self.user_id.group_ids)
        # Remove role1 from the user
        self.user_id.role_line_ids.filtered(lambda rl: rl.role_id.id == self.role1_id.id).unlink()
        # Check user has no groups from role1 and role2
        self.assertFalse(role1_groups <= self.user_id.group_ids)
        self.assertFalse(role2_groups <= self.user_id.group_ids)
    ...
    def test_default_user_roles(self):
        ...
        user = self.user_model.create({"name": "USER TEST (DEFAULT ROLES)", "login": "user_test_default_roles"})
    ...
    def test_role_multicompany(self):
        ...
        self.multicompany_user_1.write({"group_ids": [fields.Command.set(self.env.ref("base.group_user").ids)]})
        # Check that the user cannot read multicompany data again since it lost
        # its admin privileges
        with self.assertRaisesRegex(AccessError, "You are not allowed to access 'User Role'"):
            role.read()
    ...
    def test_group_groups_into_role(self):
        ...
        wizard = self.wiz_model.with_context(active_ids=user_group_ids).create({"name": "Test Role"})
