...
class ResUsers(models.Model):
    ...
    def set_groups_from_roles(self, force=False):
        ...
        for user in self:
            ...
            if admin_group and admin_group.id in user.group_ids.ids and admin_group.id not in group_ids:
                other_admins = self.sudo().search_count([("id", "!=", user.id), ("group_ids", "in", admin_group.id)])
                if other_admins == 0:
