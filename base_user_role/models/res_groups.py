...
class ResGroups(models.Model):
    ...
    parent_ids = fields.Many2many(
        ...
        "gid",
        string="Parents",
        help="Inverse relation for the Inherits field. " "The groups from which this group is inheriting",
    )
    ...
    def _compute_trans_parent_ids(self):
        for group in self:
            group.trans_parent_ids = group.parent_ids | group.parent_ids.trans_parent_ids
    ...
    def action_view_roles(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id("base_user_role.action_res_users_role_tree")
        action["context"] = {}
        if len(self.role_ids) > 1:
            action["domain"] = [("id", "in", self.role_ids.ids)]
        elif self.role_ids:
            form_view = [(self.env.ref("base_user_role.view_res_users_role_form").id, "form")]
            if "views" in action:
