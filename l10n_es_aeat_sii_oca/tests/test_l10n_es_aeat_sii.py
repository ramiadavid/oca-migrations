...
class TestL10nEsAeatSiiBase(TestL10nEsAeatModBase, TestL10nEsAeatCertificateBase):
    ...
    def _compare_sii_dict(self, json_file, inv_type, lines, extra_vals=None, module=None):
        ...
        domain = Domain("code", "=", "01") & Domain("type", "=", "sale" if "out" in inv_type else "purchase")
    ...
    def setUpClass(cls):
        ...
        cls.account_expense = cls.env.ref(f"account.{cls.company.id}_account_common_600")
...
class TestL10nEsAeatSii(TestL10nEsAeatSiiBase):
    ...
    def setUpClass(cls):
        ...
        cls.tax_agencies = cls.env["aeat.tax.agency"].search(Domain("sii_wsdl_out", "!=", False))
    ...
    def test_intracomunitary_customer_vat_country_prefix_precedence(self):
        ...
        invoice = self.invoice.copy({"partner_id": partner.id, "fiscal_position_id": self.fp_intra.id})
        invoice.action_post()
        sii_info = invoice._get_aeat_invoice_dict()
        self.assertEqual(
            sii_info["FacturaExpedida"]["Contraparte"],
    ...
    def test_intracomunitary_customer_valid_vat(self):
        ...
        invoice = self.invoice.copy({"partner_id": eu_customer.id, "fiscal_position_id": self.fp_intra.id})
        invoice.action_post()
        sii_info = invoice._get_aeat_invoice_dict()
        self.assertEqual(
            sii_info["FacturaExpedida"]["Contraparte"],
    ...
    def test_send_sii_wizard(self):
        ...
        (invoice + invoice_sii_failed + invoice_sii_modified).write({"sii_send_date": False})
