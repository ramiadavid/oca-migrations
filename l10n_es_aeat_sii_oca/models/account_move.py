...
class AccountMove(models.Model):
    ...
    def _compute_dua_invoice(self):
        for invoice in self:
            ...
            invoice.sii_dua_invoice = invoice.line_ids.filtered(lambda x, taxes=taxes: bool(taxes & x.tax_ids))
    ...
    def _get_sii_tax_req(self, tax):
        ...
        if len(req_tax) > 1:
            raise UserError(self.env._("There's a mismatch in taxes for RE. Check them."))
        return req_tax
    ...
    def button_cancel(self):
        if not self._cancel_send_to_sii():
            raise UserError(self.env._("You cannot cancel this invoice. Please, try again later."))
