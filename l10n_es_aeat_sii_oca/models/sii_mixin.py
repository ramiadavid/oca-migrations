...
class SiiMixin(models.AbstractModel):
    ...
    def _get_dua_sii_exempt_taxes(self):
        self.ensure_one()
        return self.company_id._get_taxes_from_xmlids(["account_tax_template_p_dua_exempt"])
    ...
    def _get_aeat_header(self, tipo_comunicacion=False, cancellation=False):
        ...
        if not self.company_id.vat:
            raise UserError(self.env._("No VAT configured for the company '%s'", self.company_id.name))
        header = {
        ...
        return self.filtered(
            lambda document: (
                ...
                and (
                    ...
                    or (document.aeat_state != "cancelled" and document.sii_needs_cancel)
                )
            )
        )
    ...
    def _aeat_check_exceptions(self):
        ...
        if self.company_id.sii_enabled:
            ...
            if (
                ...
                and not partner.vat
                and not (partner.aeat_identification_type and partner.aeat_identification)
        ...
        for tax_line in tax_lines.values():
            ...
            if tax in (taxes_sfess + taxes_sfesse + taxes_sfesns):
                ...
                if tax in taxes_sfesns:
                    default_no_taxable_cause = self._get_no_taxable_cause()
                    nsub_dict = service_breakdown.setdefault("NoSujeta", {default_no_taxable_cause: 0})
                    nsub_dict[default_no_taxable_cause] += tax_line["base"]
    ...
    def _get_sii_identifier(self):
        ...
        if partner.vat and len(partner.vat) > 1 and partner.vat[1].isalpha():
            ...
            vat_country_code = partner._map_aeat_country_iso_code(partner.country_id) or country_code
        # Limpiar alfanum
        if identifier:
        ...
        if partner._map_aeat_country_code(vat_country_code) in partner._get_aeat_europe_codes():
            full_eu_vat = identifier if identifier.startswith(vat_country_code) else vat_country_code + identifier
        if gen_type == 1:
