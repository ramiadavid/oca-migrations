...
class AeatSiiMap(models.Model):
    ...
    def _unique_date_range_one(self):
        ...
        if self.date_from and self.date_to:
            domain &= (
                (Domain("date_from", "<=", self.date_to) & Domain("date_from", ">=", self.date_from))
                | (Domain("date_to", "<=", self.date_to) & Domain("date_to", ">=", self.date_from))
                | (Domain("date_from", "=", False) & Domain("date_to", ">=", self.date_from))
                | (Domain("date_to", "=", False) & Domain("date_from", "<=", self.date_to))
            )
        elif self.date_from:
