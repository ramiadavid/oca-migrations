...
class L10nEsAeatMod303Report(models.Model):
    ...
    def _compute_resultado_liquidacion(self):
        for report in self:
            report.resultado_liquidacion = report.currency_id.round(
                report.casilla_69 - report.previous_result + report.casilla_109 - report.casilla_112
            )
    ...
    def check_qty(self):
        if self.filtered(
            lambda x: (
                float_compare(x.cuota_compensar, 0, precision_digits=x.currency_id.decimal_places) < 0
                or float_compare(
