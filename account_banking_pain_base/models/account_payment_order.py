...
class AccountPaymentOrder(models.Model):
    ...
    @api.depends(
        "payment_mode_id",
        "company_partner_bank_id.account_type",
        "company_partner_bank_id.sanitized_account_number",
        "payment_line_ids.currency_id",
        "payment_line_ids.partner_bank_id.account_type",
        "payment_line_ids.partner_bank_id.sanitized_account_number",
    )
    def _compute_sepa(self):
        ...
        for order in self:
            ...
            if payment_method.pain_version:
                ...
                sepa = True
                if order.company_partner_bank_id and order.company_partner_bank_id.account_type != "iban":
                    sepa = False
                if (
                    order.company_partner_bank_id
                    and order.company_partner_bank_id.sanitized_account_number[:2] not in sepa_list
                ):
                    sepa = False
                for pline in order.payment_line_ids:
                    if pline.currency_id != eur:
                        sepa = False
                        break
                    if pline.partner_bank_id and pline.partner_bank_id.account_type != "iban":
                        sepa = False
                        break
                    if pline.partner_bank_id and pline.partner_bank_id.sanitized_account_number[:2] not in sepa_list:
                        sepa = False
                        break
    ...
    def _prepare_field(self, field_name, field_value, eval_ctx, max_size=0, gen_args=None):
        ...
        try:
            ...
            error_msg_prefix = self.env._("Cannot compute the field '%(field_name)s'.", field_name=field_name)
    ...
    def finalize_sepa_file_creation(self, xml_root, gen_args):
        ...
        logger.debug(f"Generated SEPA XML file in format {gen_args['pain_flavor']} below")
    ...
    def generate_address_block(self, parent_node, partner, gen_args):  # noqa: C901
        ...
        if (pain_flavor.startswith("pain.001.001.") or pain_flavor.startswith("pain.008.001.")) and (
            partner.zip or partner.city
        ):
            val = ""
            if partner.zip:
                val += self._prepare_field(
    ...
    def generate_party_block(self, parent_node, party_type, order, partner_bank, gen_args, bank_line=None):
        ...
        name = "partner_bank.holder_name or partner_bank.partner_id.name"
