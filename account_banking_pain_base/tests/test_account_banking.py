...
class TestPainBase(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        Journal = cls.env["account.journal"].search([("company_id", "=", cls.env.company.id)], limit=1)
    ...
    def test_generate_party_agent_with_bic(self):
        ...
        res = self.order.generate_party_agent(parent, "Cdtr", "B", self.partner_bank, self.gen_args)
        self.assertTrue(res)
    ...
    def test_generate_party_acc_number_iban(self):
        parent = etree.Element("CdtTrfTxInf")
        res = self.order.generate_party_acc_number(parent, "Dbtr", "B", self.partner_bank, self.gen_args)
        self.assertTrue(res)
    ...
    def test_except_messages_prepare_field(self):
        partner_bank = self.env["res.partner.bank"].create(
            {
                ...
                "account_number": "ES1299999999509999999999",
            }
        )
        ...
        error_messages = self.order.except_messages_prepare_field(eval_ctx, "Test Field")
        self.assertIn(f"Payment Line has reference '{payment_line.name}'.", error_messages)
        self.assertIn(
    ...
    def test_prepare_field_with_convert_to_ascii(self):
        ...
        result = self.order._prepare_field(field_name, field_value, eval_ctx, gen_args=gen_args)
        ...
        result = self.order._prepare_field(field_name, long_value, eval_ctx, max_size=35, gen_args=gen_args)
