...
class TestAccountTaxBalance(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.range_type = cls.env["date.range.type"].create({"name": "Fiscal year", "allow_overlap": False})
    ...
    def test_tax_balance(self):
        previous_taxes_ids = self.env["account.tax"].search([("has_moves", "=", True)]).ids
        ...
        tax = self.env["account.tax"].create({"name": "Tax 10.0%", "amount": 10.0, "amount_type": "percent"})
        ...
        taxes = self.env["account.tax"].search([("has_moves", "=", True), ("id", "not in", previous_taxes_ids)])
        ...
        with Form(self.env["account.move"].with_context(default_move_type="out_refund")) as refund_form:
        ...
        tax_repartition_line = tax.invoice_repartition_line_ids.filtered(lambda line: line.repartition_type == "tax")
