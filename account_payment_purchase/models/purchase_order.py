...
class PurchaseOrder(models.Model):
    ...
    supplier_partner_bank_id = fields.Many2one(
        ...
        domain="[('partner_id', 'parent_of', partner_id)," "('company_id', 'in', [False, company_id])]",
        check_company=True,
    ...
    def _get_default_supplier_partner_bank(self, partner):
        ...
        return partner.bank_ids and partner.bank_ids.sorted(lambda bank: not bank.allow_out_payment)[0].id or False
    ...
    def _compute_supplier_partner_bank(self):
        for order in self:
            if order.payment_mode_id.payment_method_id.bank_account_required and order.partner_id:
                order = order.with_company(order.company_id)
                order.supplier_partner_bank_id = order._get_default_supplier_partner_bank(order.partner_id)
