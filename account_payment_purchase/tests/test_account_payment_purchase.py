...
class TestAccountPaymentPurchase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.journal = cls.env["account.journal"].create({"name": "Test journal", "code": "TEST", "type": "general"})
        cls.payment_method_out = cls.env.ref("account.account_payment_method_manual_out")
    ...
    def test_purchase_order_invoicing(self):
        self.purchase.button_confirm()
        invoice = self.env["account.move"].create({"partner_id": self.partner.id, "move_type": "in_invoice"})
        invoice.purchase_id = self.purchase
        invoice._onchange_purchase_auto_complete()
        self.assertEqual(self.purchase.invoice_ids[0].payment_mode_id, self.payment_mode)
    ...
    def test_from_purchase_order_invoicing(self):
        ...
        self.assertEqual(result and result.get("warning", {}).get("title", False), "Warning")
