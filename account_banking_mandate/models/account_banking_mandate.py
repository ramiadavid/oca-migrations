...
class AccountBankingMandate(models.Model):
    ...
    _rec_names_search = ("unique_mandate_reference", "partner_bank_id.account_number")
    ...
    @api.depends("unique_mandate_reference", "partner_bank_id.account_number")
    def _compute_display_name(self):
        for mandate in self:
            name = mandate.unique_mandate_reference
            account_number = mandate.partner_bank_id.account_number
            if account_number:
                name = f"{name} [...{account_number[-4:]}]"
    ...
    def _compute_payment_line_ids_count(self):
        ...
        res = payment_line_model._read_group(domain=domain, groupby=["mandate_id"], aggregates=["__count"])
    ...
    def _check_dates(self):
        ...
        for mandate in self:
            ...
            if mandate.signature_date and mandate.last_debit_date and mandate.signature_date > mandate.last_debit_date:
                raise ValidationError(
                    self.env._(
                        "The mandate '%s' can't have a date of last debit " "before the date of signature.",
                        mandate.unique_mandate_reference,
                    )
                )
        ...
        for mandate in self:
            if mandate.state == "valid":
                if not mandate.signature_date:
                    raise ValidationError(
                        self.env._(
                            "Cannot validate the mandate '%s' without a date of " "signature.",
                            mandate.unique_mandate_reference,
                        )
                    )
                if not mandate.partner_bank_id:
                    raise ValidationError(
                        self.env._(
                            "Cannot validate the mandate '%s' because it is not " "attached to a bank account.",
                            mandate.unique_mandate_reference,
                        )
                    )
    ...
    def cancel(self):
        for mandate in self:
            if mandate.state not in ("draft", "valid"):
                raise UserError(self.env._("Mandate should be in draft or valid state."))
