...
class AccountPaymentLine(models.Model):
    ...
    mandate_required = fields.Boolean(related="order_id.payment_method_id.mandate_required", readonly=True)

    @api.constrains("mandate_id", "partner_bank_id")
    def _check_mandate_bank_link(self):
        for pline in self:
            if pline.mandate_id and pline.partner_bank_id and pline.mandate_id.partner_bank_id != pline.partner_bank_id:
                account_number = pline.partner_bank_id.account_number or ""
                raise ValidationError(
                    self.env._(
                        ...
                        "'%(mandate_bank_account)s').",
                        {
                            ...
                            "line_bank_account": pline.partner_bank_id.account_number or "",
                            "mandate_ref": pline.mandate_id.unique_mandate_reference or "",
                            "mandate_bank_account": account_number or "",
                        },
                    )
                )
    ...
    def _check_company_constrains(self):
        for pline in self:
            if pline.mandate_id.company_id and pline.mandate_id.company_id != pline.company_id:
                raise ValidationError(
                    self.env._(
