...
class TestInvoiceMandate(BaseCommon):
    def test_post_invoice_01(self):
        ...
        payable_move_lines = self.invoice.line_ids.filtered(lambda s: s.account_id == self.invoice_account)
    ...
    def _create_res_bank(cls, name, bic, city, country):
        return cls.env["res.bank"].create({"name": name, "bic": bic, "city": city, "country": country.id})
    ...
    def setUpClass(cls):
        ...
        cls.acme_bank = cls._create_res_bank("ACME Bank", "GEBABEBB03B", "Charleroi", cls.env.ref("base.be"))

        bank_account = cls.env["res.partner.bank"].create(
            {
                "account_number": "0023032234211123",
