...
class GeneralLedgerReportWizard(models.TransientModel):
    ...
    def _init_date_from(self):
        ...
        if today.month < int(last_fsc_month) or today.month == int(last_fsc_month) and today.day <= last_fsc_day:
    ...
    def onchange_company_id(self):
        ...
        self.only_one_unaffected_earnings_account = self._only_one_unaffected_earnings_account()
        if self.company_id and self.date_range_id.company_id and self.date_range_id.company_id != self.company_id:
        ...
        if self.company_id and self.partner_ids:
            self.partner_ids = self.partner_ids.filtered(lambda p: p.company_id == self.company_id or not p.company_id)
        if self.company_id and self.account_ids:
            if self.receivable_accounts_only or self.payable_accounts_only:
                self.onchange_type_accounts_only()
            else:
                self.account_ids = self.account_ids.filtered(lambda a: self.company_id in a.company_ids)
        if self.company_id and self.cost_center_ids:
            self.cost_center_ids = self.cost_center_ids.filtered(lambda c: c.company_id == self.company_id)
        res = {
            "domain": {
                "account_ids": [],
                "partner_ids": [],
    ...
    def _check_company_id_date_range_id(self):
        for rec in self.sudo():
            if rec.company_id and rec.date_range_id.company_id and rec.company_id != rec.date_range_id.company_id:
                raise ValidationError(
                    self.env._("The Company in the General Ledger Report Wizard and in " "Date Range must be the same.")
                )
    ...
    def _compute_unaffected_earnings_account(self):
        for record in self:
            record.unaffected_earnings_account = record.company_id.get_unaffected_earnings_account()
