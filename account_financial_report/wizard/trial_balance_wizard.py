...
class TrialBalanceReportWizard(models.TransientModel):
    ...
    grouped_by = fields.Selection(selection=[("analytic_account", "Analytic Account")], default=False)
    ...
    def _check_show_hierarchy_level(self):
        for rec in self:
            if rec.show_hierarchy and rec.show_hierarchy_level <= 0:
                raise UserError(self.env._("The hierarchy level to filter on must be greater than 0."))
        ...
        for rec in self.sudo():
            if rec.company_id and rec.date_range_id.company_id and rec.company_id != rec.date_range_id.company_id:
                raise ValidationError(
                    self.env._("The Company in the Trial Balance Report Wizard and in " "Date Range must be the same.")
                )
