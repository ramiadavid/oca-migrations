...
class JournalLedgerReportWizard(models.TransientModel):
    ...
    journal_ids = fields.Many2many(comodel_name="account.journal", string="Journals", required=False)
    ...
    def onchange_company_id(self):
        ...
        if self.company_id and self.journal_ids:
            self.journal_ids = self.journal_ids.filtered(lambda p: p.company_id == self.company_id or not p.company_id)
