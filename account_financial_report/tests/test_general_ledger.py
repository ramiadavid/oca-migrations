...
class TestGeneralLedgerReport(AccountTestInvoicingCommon):
    ...
    def _add_move(
        ...
        journal = self.env["account.journal"].search([("company_id", "=", self.env.user.company_id.id)], limit=1)
    ...
    def _get_report_lines(self, with_partners=False, account_ids=False):
        ...
        res_data = self.env["report.account_financial_report.general_ledger"]._get_report_values(general_ledger, data)
        return res_data
    ...
    def test_01_account_balance(self):
        ...
        check_receivable_account = self.check_account_in_report(self.receivable_account.id, general_ledger)
        ...
        self.assertTrue(self.check_account_in_report(self.unaffected_account.id, general_ledger))
        ...
        receivable_init_balance = self._get_initial_balance(self.receivable_account.id, general_ledger)
        ...
        # Re Generate the general ledger line
        res_data = self._get_report_lines(account_ids=(self.receivable_account + self.income_account).ids)
        general_ledger = res_data["general_ledger"]
        self.assertTrue(self.check_account_in_report(self.receivable_account.id, general_ledger))
        self.assertTrue(self.check_account_in_report(self.income_account.id, general_ledger))
        self.assertFalse(self.check_account_in_report(self.unaffected_account.id, general_ledger))
    ...
    def test_02_partner_balance(self):
        ...
        general_ledger = res_data["general_ledger"]
        check_partner = self.check_partner_in_report(self.receivable_account.id, self.partner.id, general_ledger)
    ...
    def test_03_unaffected_account_balance(self):
        ...
        res_data = self._get_report_lines()
        general_ledger = res_data["general_ledger"]
        check_unaffected_account = self.check_account_in_report(self.unaffected_account.id, general_ledger)
        self.assertTrue(check_unaffected_account)

        # Check the initial and final balance
        unaffected_init_balance = self._get_initial_balance(self.unaffected_account.id, general_ledger)
    ...
    def test_analytic_distribution_xlsx(self):
        ...
        analytic_account_1 = self.env["account.analytic.account"].create({"name": "Analytic 1", "plan_id": plan_1.id})
        analytic_account_2 = self.env["account.analytic.account"].create({"name": "Analytic 2", "plan_id": plan_2.id})
        analytic_account_3 = self.env["account.analytic.account"].create({"name": "Analytic 3", "plan_id": plan_1.id})
        analytic_account_4 = self.env["account.analytic.account"].create({"name": "Analytic 4", "plan_id": plan_2.id})
        analytic_account_5 = self.env["account.analytic.account"].create({"name": "Analytic 5", "plan_id": plan_3.id})
    ...
    def test_05_onchange_account_range_no_typeerror(self):
        ...
        self.assertIn(acc_to, wizard.account_ids, "The ending account should be in the filter.")
        self.assertNotIn(
            acc_out,
            wizard.account_ids,
