...
class TestAccountAgeReportConfiguration(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.account_age_report_config = cls.env["account.age.report.configuration"].create(
            {
    ...
    def test_check_line_ids_constraint(self):
        with self.assertRaises(ValidationError):
            self.env["account.age.report.configuration"].create({"name": "Interval configuration", "line_ids": False})
