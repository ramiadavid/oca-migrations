...
class AgedPartnerBalanceXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        if not report.show_move_line_details:
            return self._get_report_columns_without_move_line_details(report, column_index=3)
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.aged_partner_balance"]._get_report_values(report, data)
        ...
        for col_pos, column in report_data["columns"].items():
            if col_pos == col_pos_footer_label or column.get(field_name):
                ...
                if cell_type == "string" or col_pos == col_pos_footer_label:
                    report_data["sheet"].write_string(report_data["row_pos"], col_pos, value or "", string_format)
                elif cell_type == "amount":
                    number = float(value)
                    if amount_is_percent:
                        number /= 100
                    report_data["sheet"].write_number(report_data["row_pos"], col_pos, number, amount_format)
            else:
                report_data["sheet"].write_string(report_data["row_pos"], col_pos, "", string_format)
