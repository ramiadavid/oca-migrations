...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_initial_balances_bs_ml_domain(
        ...
        accounts = self.env["account.account"].with_context(active_test=False).search(accounts_domain)
    ...
    def _get_pl_initial_balance(
        ...
        for initial_balance in initial_balances:
            ...
            if foreign_currency:
                pl_initial_currency_balance += round(initial_balance["amount_currency:sum"], 2)
        return pl_initial_balance, pl_initial_currency_balance

    @api.model
    def _compute_account_amount(self, total_amount, tb_initial_acc, tb_period_acc, foreign_currency):
        ...
        for tb in tb_initial_acc:
            ...
            if acc_id not in total_amount.keys():
                ...
                total_amount[acc_id]["group_by_data"][0] = self._prepare_total_amount(tb, foreign_currency)
            else:
                total_amount[acc_id]["initial_balance"] += tb["balance"] if "balance" in tb else tb["balance:sum"]
                total_amount[acc_id]["ending_balance"] += tb["balance"] if "balance" in tb else tb["balance:sum"]
                if foreign_currency:
                    amount_currency = tb["amount_currency"] if "amount_currency" in tb else tb["amount_currency:sum"]
                    total_amount[acc_id]["initial_currency_balance"] += round(amount_currency, 2)
                    total_amount[acc_id]["ending_currency_balance"] += round(amount_currency, 2)
                    ...
                    for gb_key in list(tb["group_by_data"]):
                        ...
                        if "group_by_data" in total_amount[acc_id]:
                            if gb_key not in total_amount[acc_id]["group_by_data"]:
                                total_amount[acc_id]["group_by_data"][gb_key] = self._prepare_total_amount(
                                    tb2, foreign_currency
                                )
                            else:
                                total_amount[acc_id]["group_by_data"][gb_key]["initial_balance"] += tb2["balance"]
                                total_amount[acc_id]["group_by_data"][gb_key]["ending_balance"] += tb2["balance"]
                                if foreign_currency:
                                    total_amount[acc_id]["group_by_data"][gb_key]["initial_currency_balance"] = round(
                                        tb2["amount_currency"], 2
                                    )
                                    total_amount[acc_id]["group_by_data"][gb_key]["ending_currency_balance"] += round(
                                        tb2["amount_currency"], 2
                                    )
        return total_amount
    ...
    @api.model
    def _compute_acc_prt_amount(self, total_amount, tb, acc_id, prt_id, foreign_currency):
        # Add keys to dict if not exists
        if acc_id not in total_amount:
            total_amount[acc_id] = self._prepare_total_amount(tb, foreign_currency)
        if prt_id not in total_amount[acc_id]:
            total_amount[acc_id][prt_id] = self._prepare_total_amount(tb, foreign_currency)
        else:
            # Increase balance field values
            total_amount[acc_id][prt_id]["initial_balance"] += tb["balance"] if "balance" in tb else tb["balance:sum"]
            total_amount[acc_id][prt_id]["ending_balance"] += tb["balance"] if "balance" in tb else tb["balance:sum"]
            if foreign_currency:
                amount_currency = tb["amount_currency"] if "amount_currency" in tb else tb["amount_currency:sum"]
                total_amount[acc_id][prt_id]["initial_currency_balance"] += round(amount_currency, 2)
                total_amount[acc_id][prt_id]["ending_currency_balance"] += round(amount_currency, 2)
    ...
    @api.model
    def _compute_partner_amount(self, total_amount, tb_initial_prt, tb_period_prt, foreign_currency):
        partners_ids = set()
        partners_data = {}
        for tb in tb_period_prt:
            ...
            if prt_id not in partners_ids:
                partner_name = tb["partner_id"][1] if tb["partner_id"] else self.env._("Missing Partner")
                partners_data.update({prt_id: {"id": prt_id, "name": partner_name}})
            if prt_id not in total_amount[acc_id]:
                total_amount[acc_id][prt_id] = self._prepare_total_amount(tb, foreign_currency)
            else:
                total_amount[acc_id][prt_id]["ending_balance"] += balance
            total_amount[acc_id][prt_id]["credit"] += tb["credit"] if "credit" in tb else tb["credit:sum"]
            total_amount[acc_id][prt_id]["debit"] += tb["debit"] if "debit" in tb else tb["debit:sum"]
        ...
        for tb in tb_initial_prt:
            ...
            if prt_id not in partners_ids:
                partner_name = tb["partner_id"][1] if tb["partner_id"] else self.env._("Missing Partner")
                partners_data.update({prt_id: {"id": prt_id, "name": partner_name}})
            total_amount = self._compute_acc_prt_amount(total_amount, tb, acc_id, prt_id, foreign_currency)
    ...
    def _get_data(
        ...
        for account in accounts:
            tb_initial_acc.append({"account_id": account.id, "balance": 0.0, "amount_currency": 0.0})
        ...
        for account_rg in tb_initial_acc_rg:
            ...
            if element:
                ...
                if "__context" in account_rg and "group_by" in account_rg["__context"]:
                    ...
                    gb_data = {}
                    account_rg_grouped = self.env["account.move.line"].formatted_read_group(
        ...
        total_amount = {}
        partners_data = []
        total_amount = self._compute_account_amount(total_amount, tb_initial_acc, tb_period_acc, foreign_currency)
        if show_partner_details:
        ...
        if unaffected_id:
            ...
            if grouped_by:
                ...
                if foreign_currency and "amount_currency" not in total_amount[unaffected_id]:
                    total_amount[unaffected_id]["amount_currency"] = 0
                group_by_data_item = self._prepare_total_amount(total_amount[unaffected_id], foreign_currency)
        ...
        if unaffected_id:
            ...
            if grouped_by:
                total_amount[unaffected_id]["group_by_data"][0]["ending_balance"] = total_amount[unaffected_id][
                    "ending_balance"
                ]
                total_amount[unaffected_id]["group_by_data"][0]["initial_balance"] = total_amount[unaffected_id][
                    "initial_balance"
                ]
                if foreign_currency:
                    total_amount[unaffected_id]["group_by_data"][0]["ending_currency_balance"] = total_amount[
                        unaffected_id
                    ]["ending_currency_balance"]
                    total_amount[unaffected_id]["group_by_data"][0]["initial_currency_balance"] = total_amount[
                        unaffected_id
    ...
    def _get_data_grouped(self, total_amount, accounts_data, foreign_currency):
        ...
        for aa_id in analytic_account_ids:
            ...
            trial_balance[aa_id]["name"] = (
                aa_data[aa_id]["display_name"] if aa_id in aa_data else self.env._("Without analytic account")
            )
    ...
    def _get_hierarchy_groups(self, group_ids, groups_data, foreign_currency):
        ...
        groups = self.env["account.account"].browse(group_ids).sorted(key=lambda x: x.complete_code)
        for group in groups:
            group_id = group.id
            ...
            while parent_id:
                ...
                if foreign_currency:
                    groups_data[group_id]["initial_currency_balance"] += groups_data[group_id][
                        "initial_currency_balance"
                    ]
                    groups_data[group_id]["ending_currency_balance"] += groups_data[group_id]["ending_currency_balance"]
    ...
    def _get_groups_data(self, accounts_data, total_amount, foreign_currency):
        ...
        accounts = self.env["account.account"].with_context(active_test=False).browse(accounts_ids)
        account_group_relation = {}
        for account in accounts:
            accounts_data[account.id]["complete_code"] = (
                account.parent_id.complete_code + " / " + account.code if account.parent_id.id else ""
            )
            if account.parent_id.id:
                if account.parent_id.id not in account_group_relation.keys():
    ...
    def _get_computed_groups_data(self, accounts_data, total_amount, foreign_currency):
        # 20.0: el "grupo" es una cuenta con hijas dentro de la jerarquia de cuentas
        groups = self.env["account.account"].search([("is_account_group", "=", True)])
        groups_data = {}
        for group in groups:
            len_group_code = len(group.code)
            groups_data.update(
                {
                    group.id: {
                        "id": group.id,
            ...
            for account in accounts_data.values():
                if group.code == account["code"][:len_group_code]:
                    acc_id = account["id"]
                    group_id = group.id
                    groups_data[group_id]["initial_balance"] += total_amount[acc_id]["initial_balance"]
                    ...
                    groups_data[group_id]["ending_balance"] += total_amount[acc_id]["ending_balance"]
                    if foreign_currency:
    ...
    def _get_report_values(self, docids, data):
        ...
        if not show_partner_details:
                ...
                accounts_data[account_id].update(
                    {
                        ...
                        "group_by": (
                            total_amount[account_id]["group_by"] if "group_by" in total_amount[account_id] else False
                        ),
                        "group_by_data": (
            ...
            if foreign_currency:
                for account_id in accounts_data.keys():
                    total_amount[account_id]["currency_id"] = accounts_data[account_id]["currency_id"]
                    total_amount[account_id]["currency_name"] = accounts_data[account_id]["currency_name"]
        res.update(
            {
                "doc_ids": [wizard_id],
