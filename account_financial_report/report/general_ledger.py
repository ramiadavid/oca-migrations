...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_taxes_data(self, taxes_ids):
        taxes = (
            ...
            .with_context(active_test=False)
            .search_fetch([("id", "in", taxes_ids)], ["amount", "amount_type", "display_name"])
        )
        taxes_data = {}
        for tax in taxes:
            taxes_data.update(
    ...
    def _get_initial_balances_pl_ml_domain(self, account_ids, company_id, date_from, fy_start_date, base_domain):
        accounts_domain = [
            ("company_ids", "in", [company_id]),
    ...
    def _get_initial_balance_fy_pl_ml_domain(self, account_ids, company_id, fy_start_date, base_domain):
        accounts_domain = [
            ("company_ids", "in", [company_id]),
            ("include_initial_balance", "=", False),
        ]
    ...
    def _get_pl_initial_balance(self, account_ids, company_id, fy_start_date, foreign_currency, base_domain):
        domain = self._get_initial_balance_fy_pl_ml_domain(account_ids, company_id, fy_start_date, base_domain)
    ...
    def _prepare_gen_ld_data_item(self, gl):
        ...
        for key_bal in ["init_bal", "fin_bal"]:
            ...
            for key_field in ["credit", "debit", "balance", "bal_curr"]:
                ...
                res[key_bal][key_field] = gl[field_name] if field_name in gl else gl[f"{field_name}:sum"]
        return res
        ...
        if unaffected_id:
            ...
            for key_bal in ["init_bal", "fin_bal"]:
                ...
                for field_name in fields_balance:
                    data[unaffected_id][key_bal][field_name] += pl_initial_balance[field_name]
        return data
    ...
    def _get_reconciled_after_date_to_ids(self, full_reconcile_ids, date_to):
        ...
        reconciled_after_date_to = self.env["account.partial.reconcile"].search_read(domain=domain, fields=fields)
        rec_after_date_to_ids = list(map(operator.itemgetter("full_reconcile_id"), reconciled_after_date_to))
    ...
    def _prepare_ml_items(self, move_line, grouped_by):
        ...
        if grouped_by == "partners":
            ...
            item_name = move_line["partner_id"][1] if move_line["partner_id"] else self.env._("Missing Partner")
    ...
    def _get_period_ml_data(
        ...
        move_lines = self.env["account.move.line"].search_read(domain=domain, fields=ml_fields, order="date,move_name")
        journal_ids = set()
            ...
            if acc_id in acc_prt_account_ids:
                ...
                for item in item_ids:
                    ...
                    if item_id not in gen_ld_data[acc_id]:
                        ...
                        gen_ld_data[acc_id][item_id] = self._initialize_data(foreign_currency)
                        gen_ld_data[acc_id][item_id]["id"] = item_id
                        gen_ld_data[acc_id][item_id]["name"] = item["name"]
                    gen_ld_data[acc_id][item_id][ml_id] = self._get_move_line_data(move_line)
                    gen_ld_data[acc_id][item_id]["fin_bal"]["credit"] += move_line["credit"]
                    gen_ld_data[acc_id][item_id]["fin_bal"]["debit"] += move_line["debit"]
                    gen_ld_data[acc_id][item_id]["fin_bal"]["balance"] += move_line["balance"]
                    if foreign_currency:
                        gen_ld_data[acc_id][item_id]["fin_bal"]["bal_curr"] += move_line["amount_currency"]
            else:
        ...
        rec_after_date_to_ids = self._get_reconciled_after_date_to_ids(full_reconcile_data.keys(), date_to)
        return (
            gen_ld_data,
            accounts_data,
            journals_data,
    ...
    def _recalculate_cumul_balance(self, move_lines, last_cumul_balance, rec_after_date_to_ids):
        for move_line in move_lines:
            ...
            if move_line["rec_id"] in rec_after_date_to_ids:
                move_line["rec_name"] = "(" + self.env._("future") + ") " + move_line["rec_name"]
        return move_lines
    ...
    def _get_list_grouped_item(self, data, account, rec_after_date_to_ids, hide_account_at_0, rounding):
        list_grouped = []
    ...
    def _get_report_values(self, docids, data):
        ...
        if centralize:
            for account in general_ledger:
                if account["centralized"]:
                    centralized_ml = self._get_centralized_ml(account, date_to, grouped_by)
        ...
        for gl_item in general_ledger:
            if not foreign_currency:
                continue
            if not gl_item["currency_id"] or gl_item["currency_id"] != company.currency_id.id:
        ...
        for gl_item in general_ledger:
            ...
            if "move_lines" in gl_item:
                for ml in gl_item["move_lines"]:
                    ml_currency_id = ml["currency_id"][0] if ml["currency_id"] else False
