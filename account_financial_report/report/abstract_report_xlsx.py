...
class AbstractReportXslx(models.AbstractModel):
    ...
    def _get_currency_from_company(self, data=None):
        ...
        wizard = self.env[model].browse(active_id).exists() if model and active_id else False
    ...
    def _define_formats(self, workbook, report_data, data=None):
        ...
        report_data["formats"] = {
            ...
            "format_right_bold_italic": workbook.add_format({"align": "right", "bold": True, "italic": True}),
            "format_header_left": workbook.add_format({"bold": True, "border": True, "bg_color": "#FFFFCC"}),
            ...
            "format_header_amount": workbook.add_format({"bold": True, "border": True, "bg_color": "#FFFFCC"}),
            "format_amount": workbook.add_format(),
            "format_amount_bold": workbook.add_format({"bold": True}),
            "format_percent_bold_italic": workbook.add_format({"bold": True, "italic": True}),
        }
        report_data["formats"]["format_amount"].set_num_format("#,##0." + "0" * currency_id.decimal_places)
        report_data["formats"]["format_header_amount"].set_num_format("#,##0." + "0" * currency_id.decimal_places)
        report_data["formats"]["format_percent_bold_italic"].set_num_format("#,##0.00%")
        report_data["formats"]["format_amount_bold"].set_num_format("#,##0." + "0" * currency_id.decimal_places)
            ...
            if cell_type == "string":
                ...
                if line_dict.get("currency_name", False):
                    ...
                    report_data["sheet"].write_number(
                        report_data["row_pos"],
                        col_pos,
                        self._round_value_by_currency(value, line_dict.get("currency_id")),
                        format_amt,
                    )
                ...
                if cell_type == "string":
                    ...
                    if my_object["currency_id"]:
                        ...
                        report_data["sheet"].write_number(
                            report_data["row_pos"],
                            col_pos,
                            self._round_value_by_currency(value, my_object.get("currency_id")),
                            format_amt,
                        )
