...
class VATReport(models.AbstractModel):
    ...
    def _get_tax_group_data(self, tax_group_ids):
        tax_groups = self.env["account.tax.group"].search_fetch([("id", "in", tax_group_ids)], ["name", "sequence"])
        tax_group_data = {}
    ...
    def _get_tags_data(self, tags_ids):
        tags = self.env["account.account.tag"].search_fetch([("id", "in", tags_ids)], ["name"])
        tags_data = {}
        for tag in tags:
    ...
    def _get_report_values(self, docids, data):
        ...
        vat_report_data, tax_data = self._get_vat_report_data(company_id, date_from, date_to, only_posted_moves)
