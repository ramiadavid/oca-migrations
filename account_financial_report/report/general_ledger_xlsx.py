...
class GeneralLedgerXslx(models.AbstractModel):
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.general_ledger"]._get_report_values(report, data)
        ...
        for account in general_ledger:
            ...
            if "list_grouped" not in account:
                ...
                for line in account["move_lines"]:
                        ...
                        if report.show_cost_center:
                            for account_ids, value in line["analytic_distribution"].items():
                                for account_id in account_ids.split(","):
                                    analytic_distribution += f"{analytic_data[int(account_id)]['name']} "
                                    if float_compare(value, 100, precision_digits=2) < 0:
                    ...
                    if foreign_currency and line_currency_id and line_currency_id != company_currency.id:
                    ...
                    group_item.update(
                        {
                            ...
                            "type": "partner",
                            "grouped_by": account["grouped_by"] if "grouped_by" in account else "",
                            "currency_id": accounts_data[account["id"]]["currency_id"],
                            "currency_name": accounts_data[account["id"]]["currency_name"],
                        }
                    )
