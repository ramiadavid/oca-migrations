...
class OpenItemsXslx(models.AbstractModel):
    ...
    def _get_report_filters(self, report):
        return [
            ...
            [
                ...
                self.env._("All posted entries") if report.target_move == "posted" else self.env._("All entries"),
            ],
            [
    ...
    def _calculate_amounts_by_partner(self, account_id, open_items_move_lines_data):
        ...
        for line in open_items_move_lines_data:
            ...
            total_amount[account_id][partner_id_key]["residual"] += line["amount_residual"]
        return total_amount

    def _generate_report_content_by_salesperson(self, workbook, report, data, report_data, res_data):
            ...
            for account_id in Open_items.keys():
                if partner_id in Open_items[account_id]:
                    ...
                    if Open_items[account_id]:
                        ...
                        has_lines = False
                        for partner_id_key, total_amount_dict in partner_totals.get(account_id, {}).items():
                            ...
                            if has_lines:
                                ...
                                partner_data = {
                                    "id": partner_id_key,
                                    "name": partner.name if partner else self.env._("Missing Partner"),
    ...
    def _generate_report_content_by_partner(self, workbook, report, data, report_data, res_data):
        Open_items = res_data["Open_Items"]
        accounts_data = res_data["accounts_data"]
        partners_data = res_data["partners_data"]
        journals_data = res_data["journals_data"]
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.open_items"]._get_report_values(report, data)
        show_partner_details = res_data["show_partner_details"]
        grouped_by = res_data["grouped_by"]
        if grouped_by == "salesperson" and show_partner_details:
            return self._generate_report_content_by_salesperson(workbook, report, data, report_data, res_data)
        else:
            return self._generate_report_content_by_partner(workbook, report, data, report_data, res_data)
    ...
    def write_ending_balance_from_dict(
        ...
        if type_object == "partner":
            ...
            my_object["residual"] = total_amount[account_id][partner_id]["residual"]
            label = self.env._("Ending balance")
        return super().write_ending_balance_from_dict(my_object, name, label, report_data)
