...
class JournalLedgerXslx(models.AbstractModel):
    ...
    def _get_report_filters(self, report):
        target_label_by_value = {
            value: label for value, label in self.env["journal.ledger.report.wizard"]._get_move_targets()
        }

        sort_option_label_by_value = {
            value: label for value, label in self.env["journal.ledger.report.wizard"]._get_sort_options()
        }
        ...
        return [
            ...
            [
                self.env._("Journals"),
                ", ".join([f"{report_journal.code} - {report_journal.name}" for report_journal in report.journal_ids]),
            ],
        ]

    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.journal_ledger"]._get_report_values(report, data)
    ...
    def _generate_no_group_content(self, workbook, report, res_data, report_data):
        self._generate_moves_content(workbook, "Report", report, res_data, res_data["Moves"], report_data)
    ...
    def _generate_journal_content(self, workbook, report, res_data, ledger, report_data):
        ...
        currency_name = journal.currency_id and journal.currency_id.name or journal.company_id.currency_id.name
        sheet_name = f"{journal.code} ({currency_name}) - {journal.name}"
        self._generate_moves_content(workbook, sheet_name, report, res_data, ledger["report_moves"], report_data)
