...
class AgedPartnerBalanceReport(models.AbstractModel):
    ...
    @api.model
    def _get_move_lines_domain_not_reconciled(self, company_id, account_ids, partner_ids, only_posted_moves, date_from):
        domain = [
    ...
    @api.model
    def _get_new_move_lines_domain(self, new_ml_ids, account_ids, company_id, partner_ids, only_posted_moves):
        domain = [
            ("account_id", "in", account_ids),
            ("company_id", "=", company_id),
            ("id", "in", new_ml_ids),
    ...
    def _recalculate_move_lines(
        ...
        new_move_lines = self.env["account.move.line"].search_read(domain=new_domain, fields=ml_fields)
        ...
        for move_line in move_lines:
            ...
            if "amount_currency" in move_line and (
                "currency_id" not in move_line or move_line["currency_id"] == company_currency.id
            ):
