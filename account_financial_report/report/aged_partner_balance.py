...
class AgedPartnerBalanceReport(models.AbstractModel):
    ...
    def _calculate_amounts(self, ag_pb_data, acc_id, prt_id, residual, due_date, date_at_object):
        ...
        for index, line in enumerate(interval_lines):
            ...
            interval_range = self._get_values_for_range_intervals(lower_limit, next_line.inferior_limit)
            if days_difference in interval_range or days_difference == line.inferior_limit:
    ...
    def _get_account_partial_reconciled(self, company_id, date_at_object):
        ...
        accounts_partial_reconcile = self.env["account.partial.reconcile"].search_read(domain=domain, fields=fields)
        debit_amount = {}
        debit_amount_currency = {}
        credit_amount = {}
        credit_amount_currency = {}
        for account_partial_reconcile_data in accounts_partial_reconcile:
            ...
            debit_amount_currency[debit_move_id] += account_partial_reconcile_data["debit_amount_currency"]
            ...
            credit_amount_currency[credit_move_id] += account_partial_reconcile_data["credit_amount_currency"]
            account_partial_reconcile_data.update({"debit_move_id": debit_move_id, "credit_move_id": credit_move_id})
        return (
    ...
    def _compute_maturity_date(self, ml, date_at_object):
        ...
        if due_date:
            ...
            for index, interval_line in enumerate(interval_lines):
                ...
                next_line = interval_lines[index] if index < len(interval_lines) else None
                interval_range = self._get_values_for_range_intervals(lower_limit, next_line.inferior_limit)
                if days_difference in interval_range or days_difference == interval_line.inferior_limit:
        ...
        for account in accounts_data.values():
            ...
            for prt_id in ag_pb_data[acc_id]:
                if isinstance(prt_id, int):
                    ...
                    for interval_line in interval_lines:
                        partner[interval_line] = ag_pb_data[acc_id][prt_id][interval_line]
                    if show_move_line_details:
                        move_lines = []
    ...
    def _calculate_percent(self, aged_partner_data):
        ...
        for account in aged_partner_data:
            if abs(account["residual"]) > 0.01:
                ...
                for interval_line in interval_lines:
                    account[f"percent_{interval_line.id}"] = abs(round((account[interval_line] / total) * 100, 2))
            else:
                account.update(
                    {
                        "percent_current": 0.0,
    ...
    def _get_report_values(self, docids, data):
        ...
        aged_partner_configuration = self.env["account.age.report.configuration"].browse(data["age_partner_config_id"])
        (
            ag_pb_data,
            accounts_data,
            partners_data,
            journals_data,
        ) = self.with_context(age_partner_config=aged_partner_configuration)._get_move_lines_data(
            company_id,
            account_ids,
            partner_ids,
            date_at_object,
        ...
        aged_partner_data = self.with_context(age_partner_config=aged_partner_configuration)._create_account_list(
            ag_pb_data,
            accounts_data,
            partners_data,
            journals_data,
        ...
        aged_partner_data = self.with_context(age_partner_config=aged_partner_configuration)._calculate_percent(
            aged_partner_data
        )
        res.update(
            {
