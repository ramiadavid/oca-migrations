...
class AccountAgeReportConfiguration(models.Model):
    ...
    line_ids = fields.One2many("account.age.report.configuration.line", "account_age_report_config_id")
    ...
    def _check_inferior_limit(self):
        for rec in self:
            if rec.inferior_limit <= 0:
                raise ValidationError(self.env._("Inferior Limit must be greather than zero"))
