...
class ReportFacturaeSigned(models.AbstractModel):
    ...
    def generate_report(self, ir_report, docids, data=None):
        ...
        xml_facturae, content_type = super().generate_report(ir_report, docids, data=data)
        self._validate_facturae(move, xml_facturae)
        public_crt, private_key = self.env["l10n.es.aeat.certificate"].sudo().get_certificates(move.company_id)
    ...
    def _validate_facturae(self, move, xml_string):
        facturae_schema = etree.XMLSchema(etree.parse(self._get_facturae_schema_file(move)))
        try:
    ...
    def _sign_file(self, move, request, public_cert, private_key):
        ...
        signed_properties_id = f"{signature_id}-SignedProperties{random.randint(rand_min, rand_max):05d}"
        ...
        try:
            ...
            raise ValidationError(self.env._("The provided certificate is not found in the system.")) from e
        ...
        xmlsig.template.add_reference(sign, xmlsig.constants.TransformSha1, uri="#" + key_info_id)
        ref = xmlsig.template.add_reference(sign, xmlsig.constants.TransformSha1, name=reference_id, uri="")
        ...
        etree.SubElement(signed_signature_properties, etree.QName(etsi, "SigningTime")).text = now.isoformat()
        signing_certificate = etree.SubElement(signed_signature_properties, etree.QName(etsi, "SigningCertificate"))
        signing_certificate_cert = etree.SubElement(signing_certificate, etree.QName(etsi, "Cert"))
        cert_digest = etree.SubElement(signing_certificate_cert, etree.QName(etsi, "CertDigest"))
        etree.SubElement(
            cert_digest,
        ...
        etree.SubElement(cert_digest, etree.QName(xmlsig.constants.DSigNs, "DigestValue")).text = base64.b64encode(
            hash_cert.digest()
        )
        issuer_serial = etree.SubElement(signing_certificate_cert, etree.QName(etsi, "IssuerSerial"))
        etree.SubElement(
            issuer_serial, etree.QName(xmlsig.constants.DSigNs, "X509IssuerName")
        ).text = xmlsig.utils.get_rdns_name(certificate.issuer.rdns)
        etree.SubElement(issuer_serial, etree.QName(xmlsig.constants.DSigNs, "X509SerialNumber")).text = str(
        ...
        signature_policy_id = etree.SubElement(signature_policy_identifier, etree.QName(etsi, "SignaturePolicyId"))
        sig_policy_id = etree.SubElement(signature_policy_id, etree.QName(etsi, "SigPolicyId"))
        etree.SubElement(sig_policy_id, etree.QName(etsi, "Identifier")).text = sig_policy_identifier
        etree.SubElement(sig_policy_id, etree.QName(etsi, "Description")).text = "Política de Firma Facturae v3.1"
        sig_policy_hash = etree.SubElement(signature_policy_id, etree.QName(etsi, "SigPolicyHash"))
        etree.SubElement(
            sig_policy_hash,
            etree.QName(xmlsig.constants.DSigNs, "DigestMethod"),
            attrib={"Algorithm": "http://www.w3.org/2000/09/xmldsig#sha1"},
        ...
        signer_role = etree.SubElement(signed_signature_properties, etree.QName(etsi, "SignerRole"))
        claimed_roles = etree.SubElement(signer_role, etree.QName(etsi, "ClaimedRoles"))
        etree.SubElement(claimed_roles, etree.QName(etsi, "ClaimedRole")).text = "supplier"
        ...
        etree.SubElement(data_object_format, etree.QName(etsi, "Description")).text = "Factura"
        etree.SubElement(data_object_format, etree.QName(etsi, "MimeType")).text = "text/xml"
        ctx = xmlsig.SignatureContext()

        ctx.x509 = certificate
        ctx.public_key = certificate.public_key()
        with open(private_key, "rb") as f:
            ctx.private_key = serialization.load_pem_private_key(f.read(), password=None, backend=default_backend())
        root.append(sign)
        ctx.sign(sign)
