...
class CommonTest(TestL10nEsAeatCertificateBase, TestL10nEsAeatModBase):
    ...
    def setUpClass(cls):
        ...
        self.env["res.currency.rate"].search([("currency_id", "=", self.company.currency_id.id)]).write(
            {"company_id": False}
        )
        bank_obj = self.env["res.partner.bank"]
        self.bank = bank_obj.create(
            {
                "account_number": "FR20 1242 1242 1242 1242 1242 124",
    ...
    def test_bank(self):
        ...
        self.bank.account_number = "1111"
        with self.assertRaises(exceptions.ValidationError):
            self.move.validate_facturae_fields()
        with self.assertRaises(exceptions.ValidationError):
            self.move_02.validate_facturae_fields()
        self.bank.account_number = "BE96 9988 7766 5544"
    ...
    def test_signature(self):
        ...
        self.move.action_post()
        self.move.name = "2999/99999"
        self.company.partner_id.country_id = self.env.ref("base.es")
        generated_facturae = self._create_facturae_file(self.move)
    ...
    def _check_totals(self, move, subtotal, base, tax, total):
        ...
        self.assertEqual(
            generated_facturae.xpath("//InvoiceTotals//AmountsWithheld//WithholdingReason")[0].text,
            "WithholdingReason",
        )
        version = move.get_facturae_version()
        self.assertEqual(
            generated_facturae.xpath("//InvoiceTotals//AmountsWithheld//WithholdingRate")[0].text,
            ("%.4f" if version == "3_2" else "%.8f") % 5,
        )
        self.assertEqual(
            generated_facturae.xpath("//InvoiceTotals//AmountsWithheld//WithholdingAmount")[0].text,
    ...
    def test_hide_discount(self):
        ...
        self.company.facturae_hide_discount = True
        ...
        self.company.facturae_hide_discount = False
