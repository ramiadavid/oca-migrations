...
class AccountPaymentTermHoliday(models.Model):
    ...
    def check_holiday(self):
        for record in self:
            if fields.Date.from_string(record.date_postponed) <= fields.Date.from_string(record.holiday):
                raise ValidationError(self.env._("Holiday %s can only be postponed into the future") % record.holiday)
            if (
                record.search_count(
                    [
                ...
                raise ValidationError(self.env._("Holiday %s is duplicated in current payment term") % record.holiday)
            if (
                record.search_count(
                    [
                        ("payment_id", "=", record.payment_id.id),
                ...
                raise ValidationError(
                    self.env._("Date %s cannot is both a holiday and a Postponed date") % record.holiday
                )
