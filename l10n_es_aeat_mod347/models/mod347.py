...
class L10nEsAeatMod347Report(models.Model):
    ...
    def _error_count(self, model):
        records_error_group = self.env[f"l10n.es.aeat.mod347.{model}"]._read_group(
            domain=Domain.AND([Domain("check_ok", "=", False), Domain("report_id", "in", self.ids)]),
            groupby=["report_id"],
            aggregates=["__count"],
        )
    ...
    def _create_partner_records(self, key, map_ref, partner_record=None):
        ...
        filtered_groups = list(filter(lambda d: abs(d[1]) > self.operations_limit, groups))
    ...
    def _create_cash_moves(self):
        ...
        for cash_group in cash_groups:
            ...
            if amount > self.received_cash_limit:
                move_lines = move_line_obj.search(domain & Domain("partner_id", "=", partner.id))
...
class L10nEsAeatMod347PartnerRecord(models.Model):
    ...
    def _compute_check_ok(self):
        for record in self:
            ...
            if (record.operation_key == "E" and not record.bdns_number) or (
                record.operation_key == "E" and record.bdns_number and len(record.bdns_number) != 6
            ):
                # BDSN can have a maximum of 6 digits
                errors.append(
                    self.env._("BDNS Call Number is mandatory for operation key E and it must" " have 6 digits")
                )
    ...
    def _compute_visible_bdns_number(self):
        for record in self:
            record.visible_bdns_number = record.operation_key == "E" and record.report_id.year >= 2025
    ...
    def _message_add_suggested_recipients(self, force_primary_email=False):
        ...
        for record in self:
            if record.partner_id:
                invoice_address_id = record.partner_id.address_get(["invoice"])["invoice"]
