...
class JobsTrap:
    ...
    * ``perform_enqueued_jobs``
    ...
    def assert_enqueued_job(self, method, args=None, kwargs=None, properties=None):
        ...
        * ``properties`` is a dict of job properties (priority, eta, ...)
        ...
        for call in self.calls:
            checked_properties = {key: value for key, value in call.properties.items() if key in properties}
    ...
    def _add_job(self, *args, **kwargs):
        job = Job(*args, **kwargs)
        if not job.identity_key or all(j.identity_key != job.identity_key for j in self.enqueued_jobs):
    ...
    def _filtered_enqueued_jobs(self, job_method):
        enqueued_jobs = [
            ...
            if job.func.__self__ == job_method.__self__ and job.func.__func__ == job_method.__func__
        ]
    ...
    def _format_job_call(self, call):
        ...
        if call.kwargs:
            method_args_parts.append(", ".join(f"{key}={value}" for key, value in call.kwargs.items()))
        method_args = ", ".join(method_args_parts)

        # Build properties string
        props_str = ", ".join(f"{key}={value}" for key, value in call.properties.items())

        return f"<{call.method.__self__}>." f"{call.method.__name__}({method_args}) " f"with properties ({props_str})"
...
class OdooDocTestCase(doctest.DocTestCase, _TestCase):
    ...
    def __init__(self, doctest, optionflags=0, setUp=None, tearDown=None, checker=None, seq=0):
        super().__init__(
            doctest._dt_test,
