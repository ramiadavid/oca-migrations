...
class Channel:
    ...
    def __str__(self):
        ...
        return f"{self.fullname}(C:{capacity},Q:{len(self._queue)}," f"R:{len(self._running)},F:{len(self._failed)})"
        ...
        if self.throttle and self._pause_until:
            if now < self._pause_until:
                if self.has_capacity():
                    _logger.debug(
                        "channel %s paused until %s because " "of throttle delay between jobs",
                        self,
                        self._pause_until,
                    )
                return
    ...
    * get jobs to run
    ...
    def parse_simple_config(cls, config_string):
        ...
        for channel_config_string in split_strip(config_string, ","):
            ...
            name = config_items[0]
            if not name:
                raise ValueError(f"Invalid channel config {config_string}: missing channel name")
            config["name"] = name
            if len(config_items) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid channel config {config_string}: " f"invalid capacity {capacity}") from ex
                for config_item in config_items[2:]:
                    ...
                    if len(kv) == 1:
                        ...
                        raise ValueError(
                            f"Invalid channel config {config_string}: " f"incorrect config item {config_item}"
                        )
                    if k in config:
                        raise ValueError(f"Invalid channel config {config_string}: duplicate key {k}")
                    config[k] = v
            else:
                config["capacity"] = 1
            res.append(config)
    ...
    def get_channel_by_name(self, channel_name, autocreate=False, parent_fallback=False):
    ...
    def notify(self, db_name, channel_name, uuid, seq, date_created, priority, eta, state):
        ...
        if job:
            ...
            if seq != job.seq or priority != job.priority or eta != job.eta or channel != job.channel:
