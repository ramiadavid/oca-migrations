...
def _channels():
    return os.environ.get("ODOO_QUEUE_JOB_CHANNELS") or queue_job_config.get("channels") or "root:1"
...
def _connection_info_for(db_name):
    ...
    for p in ("host", "port", "user", "password"):
        cfg = os.environ.get(f"ODOO_QUEUE_JOB_JOBRUNNER_DB_{p.upper()}") or queue_job_config.get("jobrunner_db_" + p)
...
class Database:
    ...
    def _has_queue_job(self):
        with closing(self.conn.cursor()) as cr:
            cr.execute("SELECT 1 FROM pg_tables WHERE tablename=%s", ("ir_module_module",))
            if not cr.fetchone():
            ...
            if cr.fetchone()[0] != 3:  # INSERT, DELETE, UPDATE
                _logger.error("queue_job_notify trigger is missing in db %s", self.db_name)
                return False
            return True
...
class QueueJobRunner:
    ...
    def from_environ_or_config(cls):
        scheme = os.environ.get("ODOO_QUEUE_JOB_SCHEME") or queue_job_config.get("scheme")
        host = os.environ.get("ODOO_QUEUE_JOB_HOST") or queue_job_config.get("host") or config["http_interface"]
        port = os.environ.get("ODOO_QUEUE_JOB_PORT") or queue_job_config.get("port") or config["http_port"]
        user = os.environ.get("ODOO_QUEUE_JOB_HTTP_AUTH_USER") or queue_job_config.get("http_auth_user")
        password = os.environ.get("ODOO_QUEUE_JOB_HTTP_AUTH_PASSWORD") or queue_job_config.get("http_auth_password")
        runner = cls(
    ...
    def run(self):
        ...
        while not self._stop:
            ...
            try:
                ...
                _logger.exception("exception: sleeping %ds and retrying", ERROR_RECOVERY_DELAY)
                self.close_databases()
                time.sleep(ERROR_RECOVERY_DELAY)
