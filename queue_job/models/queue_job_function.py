...
class QueueJobFunction(models.Model):
    ...
    def _inverse_edit_retry_pattern(self):
        try:
            ...
            raise exceptions.UserError(self._retry_pattern_format_error_message()) from ex
    ...
    def _inverse_edit_related_action(self):
        try:
            ...
            raise exceptions.UserError(self._related_action_format_error_message()) from ex
        ...
        for record in self:
            ...
            for value in all_values:
                try:
                    ...
                    raise exceptions.UserError(record._retry_pattern_format_error_message()) from ex
    ...
    def _check_related_action(self):
        ...
        for record in self:
            ...
            if any(key not in valid_keys for key in related_action):
                raise exceptions.UserError(record._related_action_format_error_message())
