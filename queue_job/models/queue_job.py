...
from ..job import CANCELLED, DONE, ENQUEUED, FAILED, PENDING, STARTED, STATES, WAIT_DEPENDENCIES, Job
...
class QueueJob(models.Model):
    ...
    def _compute_dependency_graph(self):
        ...
        for record in self:
            ...
            record.dependency_graph = {
                # list of ids
                "nodes": [graph_jobs_by_ids[graph_id]._dependency_graph_vis_node() for graph_id in graph.vertices()],
    ...
    def _dependency_graph_vis_node(self):
        ...
        return {
            "id": self.id,
            "title": (f"<strong>{html_escape(self.display_name)}</strong><br/>" f"{html_escape(self.func_string)}"),
    ...
    def write(self, vals):
        ...
        if vals.get("user_id"):
            different_user_jobs = self.filtered(lambda records: records.env.user.id != vals["user_id"])
        ...
        for record in different_user_jobs:
            ...
            super(QueueJob, record).write({"records": record.records.with_user(vals["user_id"])})
        return result
    ...
    def open_graph_jobs(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id("queue_job.action_queue_job")
        action.update(
            {
