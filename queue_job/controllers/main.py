...
class RunJobController(http.Controller):
    ...
    def _acquire_job(cls, env: api.Environment, job_uuid: str) -> Job | None:
        ...
        """
        env.cr.execute(
            "SELECT uuid FROM queue_job WHERE uuid=%s AND state=%s " "FOR NO KEY UPDATE SKIP LOCKED",
            (job_uuid, ENQUEUED),
        )
    ...
    def _create_graph_test_jobs(
        ...
        return f"graph uuid: {list(root_delayable._head())[0]._generated_job.graph_uuid}"
