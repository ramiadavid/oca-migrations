...
class Job:
    ...
    def _load_from_db_record(cls, job_db_record):
        ...
        job_.__reverse_depends_on_uuids.update(stored.dependencies.get("reverse_depends_on", []))
        return job_
    ...
    def __init__(
        ...
        self.job_config = self.env["queue.job.function"].sudo().job_config(self.job_function_name)
    ...
    def perform(self):
        ...
        try:
            ...
            if err.ignore_retry:
                ...
                new_exc = FailedJobError(f"Max. retries ({self.max_retries}) reached: {value or type_}")
    ...
    def store(self):
        ...
        if db_record:
            db_record.with_context(_job_edit_sentinel=edit_sentinel).write(self._store_values())
        else:
            job_model.with_context(_job_edit_sentinel=edit_sentinel).sudo().create(self._store_values(create=True))
    ...
    def _store_values(self, create=False):
        ...
        dependencies = {
            ...
            "reverse_depends_on": [children.uuid for children in self.reverse_depends_on],
        }
    ...
    def reverse_depends_on(self):
        if not self._reverse_depends_on:
            self._reverse_depends_on = Job.load_many(self.env, self.__reverse_depends_on_uuids)
    ...
    def related_action(self):
        ...
        if not isinstance(funcname, str):
            raise ValueError("related_action must be the name of the method on queue.job as string")
...
def _is_model_method(func):
    return inspect.ismethod(func) and isinstance(func.__self__.__class__, odoo.models.MetaModel)
