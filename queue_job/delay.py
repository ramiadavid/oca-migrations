    ...
    def _ensure_same_graph_uuid(jobs):
        ...
        if jobs_count == 0:
            ...
            if jobs[0].graph_uuid:
                raise ValueError(f"Job {jobs[0]} is a single job, it should not have a graph uuid")
        else:
...
class Delayable:
    ...
    def split(self, size, chain=False):
        ...
        for index, delayable in enumerate(delayables):
            delayable.set(description=f"{description} (split {index + 1}/{len(delayables)})")
    ...
    def __getattr__(self, name):
        ...
        if name in self.recordset:
            raise AttributeError(f"only methods can be delayed ({name} called on {self.recordset})")
...
class DelayableRecordset:
    ...
    def __str__(self):
        return (
            f"DelayableRecordset({self.delayable.recordset._name}" f"{getattr(self.delayable.recordset, '_ids', '')})"
        )
