...
class AccountTax(models.Model):
    ...
    def _search_l10n_es_aeat_349_operation_key(self, operator, value):
        ...
        if operator == "=" and not value:
            ...
            if is_not_in:
                operator = "in"
            map_349_lines = self.env["aeat.349.map.line"].search([("operation_key", operator, value)])
            if map_349_lines:
                taxes_xmlid = map_349_lines.mapped("tax_xmlid_ids")
                tax_ids = map_349_lines._get_tax_ids_from_xmlids(taxes_xmlid, self.env.company)
            if is_not_in:
                tax_ids = list(set(self.env["account.tax"].search([]).ids) - set(tax_ids))
    ...
    def _compute_l10n_es_aeat_349_operation_key(self):
        ...
        for tax in self:
            ...
            for line in map_349:
                taxes_ids = line._get_tax_ids_from_xmlids(line.tax_xmlid_ids, tax.company_id)
