...
class TestL10nEsAeatMod349Base(TestL10nEsAeatModBase):
    ...
    def _invoice_refund(cls, invoice, dt, price_unit=None):
        _logger.debug(f"Refund {invoice.move_type} " f"invoice: date = {dt}: price_unit = {price_unit or 150.0}")
    ...
    def test_mod349_errors(self):
        ...
        self.customer.write({"vat": "123456783", "country_id": self.env.ref("base.gr").id})
        model349_errors.button_recalculate()
        partner_record = model349_errors.partner_record_ids.filtered(lambda x: x.partner_vat == "EL123456783")
        self.assertTrue(partner_record.partner_record_ok)
        # XI VAT (Northern Ireland) without GB prefix - should be valid
        self.customer.write({"vat": "XI123456782", "country_id": self.env.ref("base.uk").id})
        model349_errors.button_recalculate()
        partner_record = model349_errors.partner_record_ids.filtered(lambda x: x.partner_id == self.customer)
        self.assertTrue(partner_record.partner_record_ok)
