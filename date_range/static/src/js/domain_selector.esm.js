...
patch(DomainSelector.prototype, {
    ...
    getOperatorEditorInfo(fieldDef) {
        ...
        patch(info, {
            extractProps({value: [operator]}) {
                ...
                const isDateField = fieldDef && (fieldDef.type === "date" || fieldDef.type === "datetime");
                ...
                if (hasDateRangeTypes) {
                    const selectedDateRange = dateRangeTypes.find(
                        (rangeType) => rangeType.id === Number(operator.split("daterange_")[1])
                    );
                    ...
                    props.options.push(
                        ...dateRangeTypes.map((rangeType) => [`daterange_${rangeType.id}`, `in ${rangeType.name}`])
                    );
                }
    ...
    update(tree) {
        ...
        const domain = tree ? Domain.and([domainFromTreeDateRange(tree), archiveDomain]).toString() : archiveDomain;
