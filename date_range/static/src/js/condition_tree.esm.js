import {condition, connector, formatValue, normalizeValue} from "@web/core/tree_editor/condition_tree";
...
function createDateRangeOperators(tree) {
    ...
    if (document.getElementsByTagName("select").length) {
        operator = document.getElementsByTagName("select")[0].selectedOptions[0].value.replace(/^"|"$/g, "");
    }
    for (let i = 0; i < processedChildren.length; i++) {
        ...
        if (
            ...
            children.push(condition(child1.path, operator, normalizeValue([child1.value, child2.value])));
            i += 1;
        } else {
