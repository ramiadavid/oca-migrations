...
class DateRangeGenerator(models.TransientModel):
    ...
    def _compute_type_id(self):
        if self.company_id and self.type_id.company_id and self.type_id.company_id != self.company_id:
    ...
    def _generate_intervals(self, batch=False):
        ...
        if not self.date_end and not self.count:
            if batch:
                return []
            raise ValidationError(self.env._("Please enter an end date, or the number of ranges to generate."))
        kwargs = dict(
        ...
        for idx, dt_start in enumerate(vals[:-1]):
            ...
            if name_expr:
                try:
                    ...
                    raise ValidationError(self.env._("Invalid name expression: %s", e)) from e
            elif name_prefix:
                names.append(name_prefix + index)
            else:
                raise ValidationError(
                    self.env._("Please set a prefix or an expression to " "generate the range names.")
                )
        return names
    ...
    def _compute_date_start(self):
        if not self.type_id:
            return
        last = self.env["date.range"].search([("type_id", "=", self.type_id.id)], order="date_end desc", limit=1)
    ...
    def _check_company_id_type_id(self):
        for rec in self.sudo():
            if rec.company_id and rec.type_id.company_id and rec.company_id != rec.type_id.company_id:
                raise ValidationError(
                    self.env._("The Company in the Date Range Generator and in " "Date Range Type must be the same.")
                )
