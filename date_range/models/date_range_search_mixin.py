...
class DateRangeSearchMixin(models.AbstractModel):
    ...
    def _search_date_range_search_id(self, operator, value):
        ...
        if isinstance(value, (OrderedSet, list, tuple, set)) and all(isinstance(v, bool) for v in value):
    ...
    def get_views(self, views, options=None):
        ...
        if "date_range_search_id" in result["models"][self._name]["fields"]:
            result["models"][self._name]["fields"]["date_range_search_id"]["string"] = self.env._("Period")
        return result
