...
class DateRangeType(models.Model):
    ...
    allow_overlap = fields.Boolean(help="If set, date ranges of same type must not overlap.", default=False)
        ...
        if not self.env.context.get("bypass_company_validation", False):
            for rec in self.sudo():
                if not rec.company_id:
                    continue
                if bool(
                    rec.date_range_ids.filtered(lambda r, drt=rec: r.company_id and r.company_id != drt.company_id)
                ):
    ...
    def autogenerate_ranges(self):
        ...
        for dr_type in self.search(
            ...
            try:
                ...
                logger.warning(f"Error autogenerating ranges for date range type " f"{dr_type.name}: {e}")
