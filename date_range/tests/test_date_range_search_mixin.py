...
class TestDateRangeSearchMixin(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry.init_models(cls.env.cr, ["test.date.range.search.mixin"], {"models_to_check": True})

        cls.env.user.lang = "en_US"
        rtype = cls.env["date.range.type"].create({"name": __name__, "company_id": False, "allow_overlap": False})
    ...
    def test_02_search_result(self):
        ...
        self.assertIn(record, self.model.search([("date_range_search_id", "in", self.ranges.ids)]))
        self.assertNotIn(
            record,
        ...
        self.assertIn(record, self.model.search([("date_range_search_id", "ilike", "1943")]))
        self.assertNotIn(record, self.model.search([("date_range_search_id", "not ilike", "1943")]))
        self.assertIn(record, self.model.search([("date_range_search_id", "not ilike", "2021")]))
        self.assertNotIn(record, self.model.search([("date_range_search_id", "ilike", "2021")]))
