...
class DateRangeTypeTest(TransactionCase):
    ...
    def test_autogeneration(self):
        ...
        self.assertEqual(dr_type.range_name_preview, f">{year_start.strftime('%d%m%Y')}<")

        self.env["date.range.type"].autogenerate_ranges()
        ranges = self.env["date.range"].search([("type_id", "=", dr_type.id)], order="date_start asc")
        ...
        self.assertEqual(ranges[-1].name, f">{next_year_month_start.strftime('%d%m%Y')}<")

        # No new ranges get generated anymore this month
        self.env["date.range.type"].autogenerate_ranges()
        self.assertEqual(
            len(ranges),
            len(self.env["date.range"].search([("type_id", "=", dr_type.id)], order="date_start asc")),
        )
