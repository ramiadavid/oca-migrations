    ...
    def _compute_display_name2(self):
        for mandate in self:
            if mandate.format == "sepa":
                mandate.display_name = f"{mandate.unique_mandate_reference} " f"({mandate.recurrent_sequence_type})"
            else:
        ...
        if expired_mandates:
            ...
            for mandate in expired_mandates:
                mandate.message_post(
                    body=self.env._(
                        "Mandate automatically set to" " expired after %s months without use.",
                        NUMBER_OF_UNUSED_MONTHS_BEFORE_EXPIRY,
                    )
                )
            logger.info(f"{len(expired_mandates)} SDD Mandate set to expired: " f"IDs {expired_mandates.ids}")
        else:
