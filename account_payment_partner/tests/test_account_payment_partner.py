...
class TestAccountPaymentPartner(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.supplier_bank.company_id = cls.company
        ...
        cls.supplier_bank_2.company_id = cls.company_2
    ...
    def test_in_invoice_onchange(self):
        ...
        # TODO: this faults because Odoo not allow to asign payment_mode form one company to invoice from other company,
        #  is this correct?
    ...
    def test_print_report(self):
        self.supplier_invoice.partner_bank_id = self.supplier_bank.id
        move_form = Form(self.supplier_invoice)
        with move_form.invoice_line_ids.new() as line_form:
            line_form.product_id = self.env.ref("product.product_product_4")
            line_form.name = "product that cost 100"
            line_form.quantity = 1.0
            line_form.price_unit = 100.0
        move_form.save()
        self.supplier_invoice.display_qr_code = True
        res = str(
            self.env["ir.actions.report"]._render_qweb_html(
                "account.account_invoices", self.supplier_invoice.ids
            )[0]
        )
    ...
    def test_account_move_payment_mode_id_default(self):
        ...
        move_form = Form(
            self.move_model.with_context(default_move_type="out_invoice").create({})
        )
