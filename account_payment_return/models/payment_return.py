...
class PaymentReturn(models.Model):
    ...
    def _check_duplicate_move_line(self):
        def append_error(error_line):
            ...
            partner_name = error_line.partner_id.name or ""
            return_name = error_line.return_id.name or ""

            label_payment_line = self.env._("Payment Line")
            label_in_return = self.env._("in Payment Return")

            msg = (
                f"{label_payment_line}: {move_names} ({partner_name}) "
                f"{label_in_return}: {return_name}"
            )
        ...
        if (not error_list) and all_move_lines:
            ...
            if duplicate_lines:
                for line in duplicate_lines:
                    append_error(line)
        if error_list:
            base_msg = self.env._("Payment reference must be unique")
            details = "\n".join(error_list)
    ...
    def _prepare_return_move_vals(self):
        ...
        """
        self.ensure_one()
        prefix = self.env._("Return")
        return {
            "name": "/",
            "ref": prefix + " " + (self.name or ""),
...
class PaymentReturnLine(models.Model):
        ...
        for line in self:
            ...
            if invoice:
                ...
                if payment_lines:
                    ...
                    if not line.concept:
                        prefix = self.env._("Invoice")
                        line.concept = f"{prefix}: {invoice.name}"
    ...
    def match_move_lines(self):
        for line in self:
            ...
            if move_lines:
                ...
                if not line.concept:
                    prefix = self.env._("Move lines")
                    names = ", ".join(move_lines.mapped("name"))
                    line.concept = prefix + ": " + names
    ...
    def match_move(self):
        for line in self:
            ...
            if move:
                ...
                if not line.concept:
                    label_move = self.env._("Move")
                    line.concept = f"{label_move}: {move.ref or ''}"
    ...
    def _prepare_return_move_line_vals(self, move):
        self.ensure_one()
        label_return = self.env._("Return")
        return {
            "name": f"{label_return} {self.return_id.name or ''}",
