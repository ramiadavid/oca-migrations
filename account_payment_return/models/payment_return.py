...
class PaymentReturn(models.Model):
    ...
    def _check_duplicate_move_line(self):
        def append_error(error_line):
            error_list.append(
                self.env._(
                    "Payment Line: %(move_names)s (%(partner_name)s) " "in Payment Return: %(return_name)s",
        ...
        if (not error_list) and all_move_lines:
            ...
            if duplicate_lines:
                for line in duplicate_lines:
                    append_error(line)
        if error_list:
            raise ValidationError(self.env._("Payment reference must be unique\n%s", "\n".join(error_list)))
    ...
    def _check_can_unlink(self):
        if self.filtered(lambda r: r.state == "done"):
            raise UserError(self.env._("You can not remove a payment return if state is 'Done'"))
    ...
    def button_match(self):
        self.mapped("line_ids").filtered(lambda x: ((not x.move_line_ids) and x.reference))._find_match()
    ...
    def action_confirm(self):
        ...
        if self.line_ids.filtered(lambda x: not x.move_line_ids):
            raise UserError(self.env._("You must input all moves references in the payment return."))
        ...
        for return_line in self.line_ids:
            ...
            return_aml = AccountMoveLine.with_context(check_move_validity=False).create(return_aml_vals)
            ...
            if return_line.expense_amount:
                ...
                AccountMoveLine.with_context(check_move_validity=False).create(expense_lines_vals)
        ...
        for to_reconcile_aml_dic in to_reconcile_aml_list:
            (to_reconcile_aml_dic["payment_aml"] + to_reconcile_aml_dic["return_aml"]).reconcile()
            to_reconcile_aml_dic["payment_aml"].mapped("matched_debit_ids").write(
                {"origin_returned_move_ids": [(6, 0, to_reconcile_aml_dic["invoice_amls"].ids)]}
            )
    ...
    def action_cancel(self):
        invoices = self.env["account.move"]
        for move_line in self.mapped("move_id.line_ids").filtered(lambda x: x.account_type == "asset_receivable"):
            for partial_line in move_line.matched_credit_ids:
                invoices |= partial_line.origin_returned_move_ids.mapped("move_id")
                lines2reconcile = partial_line.origin_returned_move_ids | partial_line.credit_move_id
...
class PaymentReturnLine(models.Model):
    ...
    reason_id = fields.Many2one(comodel_name="payment.return.reason", string="Return reason")
    ...
    move_line_ids = fields.Many2many(comodel_name="account.move.line", string="Payment Reference")
    ...
    expense_account = fields.Many2one(comodel_name="account.account", string="Charges Account")
    ...
    def _get_partner_from_move(self):
        for line in self.filtered(lambda x: not x.partner_id):
            ...
            if len(partners) > 1:
                raise UserError(self.env._("All payments must be owned by the same partner"))
    ...
    def match_invoice(self):
        for line in self:
            ...
            if invoice:
                ...
                payment_lines = invoice_line_ids.mapped("matched_debit_ids.debit_move_id")
                payment_lines |= invoice_line_ids.mapped("matched_credit_ids.credit_move_id")
                if payment_lines:
        ...
        for line in self:
            ...
            if move:
                line.move_line_ids = move.line_ids.filtered(
                    lambda aml: (aml.account_type == "asset_receivable" and aml.reconciled)
                ).ids
                if not line.concept:
                    line.concept = self.env._(
                        "Move: %(move_ref)s",
    ...
    def _find_match(self):
        ...
        lines2match = lines2match.filtered(lambda x: ((not x.move_line_ids) and x.reference))
