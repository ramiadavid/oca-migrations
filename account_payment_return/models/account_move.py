...
class AccountMove(models.Model):
    ...
    def check_payment_return(self):
        ...
        returned_invoices.filtered(lambda x: not x.returned_payment).write({"returned_payment": True})
        (self - returned_invoices).filtered("returned_payment").write({"returned_payment": False})
    ...
    def prepare_values_returned_widget(self, line_id, amount, is_return=False):
        ...
        if line_id.amount_currency and line_id.currency_id != line_id.company_id.currency_id:
    ...
    def _compute_payments_widget_reconciled_info(self):
        ...
        for move in self:
            if not move.returned_payment:
                ...
                for reconciled_aml in move._get_reconciled_amls():
                    vals_rec_payment = self.prepare_values_returned_widget(reconciled_aml, -reconciled_aml.balance)
                ...
                for move_reconcile in move_reconciles:
                    ...
                    vals_payment = self.prepare_values_returned_widget(payment, move_reconcile.amount)
                    values_returned.append(vals_payment)
                    vals_reconcile = self.prepare_values_returned_widget(payment_ret, -move_reconcile.amount, True)
        ...
        if moves_to_compute:
            return super(AccountMove, moves_to_compute)._compute_payments_widget_reconciled_info()
